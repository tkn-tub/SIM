...
function [a, cosLoc] = steeringVector(theta, phi, EnvPars)
    % theta, phi remain GLOBAL (theta from downward vertical) — unchanged callers
    dg = [sin(theta)*cos(phi); sin(theta)*sin(phi); -cos(theta)];
    ul = EnvPars.R_tilt.' * dg;
    kappa = 2*pi/EnvPars.lambda;
    ax = exp(-1i*kappa*EnvPars.d_x*ul(1)*(0:EnvPars.N_x-1)).';
    ay = exp(-1i*kappa*EnvPars.d_y*ul(2)*(0:EnvPars.N_y-1)).';
    a  = kron(ay, ax);
    cosLoc = ul(3);                       % cosine of angle from LOCAL boresight
end
