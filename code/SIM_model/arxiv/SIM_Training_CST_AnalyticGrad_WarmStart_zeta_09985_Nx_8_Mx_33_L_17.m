...
maxIter = 5000
zeta_fixed = 0.9985   % same value established from the ablation study; adjust if needed
M_x=33
M_y=M_x;
M=M_x*M_y;
L=17
...
checkpoint_every = 25;
checkpoint_path = 'SIM_training_CST_analytic_zeta_0_9985_Mx_33_L_17.mat';
...
if isfile(checkpoint_path)
    ...
    warm_start_file = fullfile('..', 'Dataset', 'SIM_training_CST_zeta_0_9985_Nx_8_Mx_33_L_17.mat');
...
for it = start_it:maxIter
    ...
    % ----- live feedback -----
    fprintf('iter %d/%d (%.1f%%), loss=%.4f, |zeta|=%d, M_x=%d, L=%d, elapsed=%.1f s\n', ...
        it, maxIter, 100*it/maxIter, Lval, abs(zeta), M_x, L, toc(t_start));
...
%% ----------------- Save final result -----------------
path = fullfile('..', 'Dataset', 'SIM_training_CST_analytic_zeta_09985_Mx_33_L_17.mat');
save(path, 'xi', 'Upsilon', 'G', 'beta', 'loss_hist', 'beta_hist', 'gap_frac_hist', ...
