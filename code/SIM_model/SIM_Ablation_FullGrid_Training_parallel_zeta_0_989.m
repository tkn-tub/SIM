...
parfor idx = 1:n_combos
    ...
    fprintf('combo %d/%d done (zeta=%.3f T_SIM=%.2f*lam, L=%d, M=%d, s_x=%.2f*lam) loss=%.4f gapfrac=%.1f%%\n', ...
        idx, n_combos, zeta_fixed,T_SIM_cur/lambda, L_cur, M_cur, s_x_cur/lambda, loss_final, 100*gapfrac_final);
end
