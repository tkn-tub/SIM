%[text] # SIM training (no CST) -- single fixed zeta, parallelized gradient
%[text] Purpose: re-run the ORIGINAL phase-only (no CST amplitude coupling)
%[text] training at whatever frequency Parameters.m currently specifies
%[text] (28 GHz, per fc=28*10^9). This script reads ALL geometry from
%[text] Parameters.m -- lambda, kappa, s_layer, d_x, A_atom all derive from
%[text] fc there, so there is no separate "switch to 28GHz" step needed here.
%[text]
%[text] Same fix pattern as the CST single-zeta script: ONE fixed zeta (no
%[text] sweep, no accidental cumulative carry-over across zeta values),
%[text] many more iterations for genuine fresh-start convergence, and the
%[text] L*M finite-difference gradient loop parallelized across workers.
%[text]
%[text] This also serves as a control experiment: if this converges cleanly
%[text] at 28GHz, the earlier non-convergence was the warm-start artifact,
%[text] not a frequency-specific issue. If it ALSO struggles, that points to
%[text] eta0/zeta needing re-tuning for the gradient scale at this frequency.
...
zeta_fixed = 0.988;   % same value established from the ablation study; adjust if needed
maxIter = 400;        % <<< increased from 40 -- tune based on the live plot
h = 1e-5;             % finite-difference step (radians)
checkpoint_every = 25;
checkpoint_path = 'SIM_training_noCST_checkpoint.mat';
...
%% ----------------- Init phases / unit-amplitude Upsilon (no CST) -----------------
...
if isfile(checkpoint_path)
    ...
    if S.maxIter ~= maxIter
        warning('Checkpoint was saved with maxIter=%d but this run uses maxIter=%d. Proceeding with the current maxIter; loss_hist/beta_hist will be resized.', ...
            S.maxIter, maxIter);
    ...
    if start_it > maxIter
        % Training already finished in a previous run -- the loop below
        % would be empty (for it = start_it:maxIter with start_it>maxIter
        % silently does nothing). Finalize directly from the saved,
        % fully-trained phases with ONE ordinary forward pass -- no
        % parfor, no pool -- rather than pointlessly starting a 32-worker
        % pool for a loop that has nothing left to do.
        ...
        path = fullfile('..', 'Dataset', 'SIM_training_noCST_single_zeta.mat');
        save(path, 'xi', 'Upsilon', 'G', 'beta', 'loss_hist', 'beta_hist', ...
             'zeta_fixed', 'maxIter', 'eta0', 'seed', 'fc', '-v7.3');
        fprintf('Finalized and saved (%s) without touching the parallel pool.\n', path);

        delete(gcp('nocreate'));   % defensive cleanup in case a pool from
                                    % an earlier process is still lingering
        return;
    end
else
    xi = cell(L,1);
    ...
    for l = 1:L
        xi{l} = 2*pi*rand(M,1);
        Upsilon{l} = diag(exp(1i*xi{l}));   % |Upsilon|=1, no amplitude coupling
    end
    eta = eta0;
...
% Submitting n_pert (~2925) individual parfor tasks EVERY iteration, for
% hundreds of iterations, means well over a million total tasks across a
% run -- a likely contributor to the parallel-runtime crash seen earlier
% (assertion in MATLAB's internal task-completion bookkeeping). Chunking
% into one task per worker cuts total task count by ~100-200x and reduces
% per-task dispatch overhead too.
nW = gcp().NumWorkers;
...
parfor c = 1:nW
    ...
    Wl = W_c.Value; WLl = WL_c.Value; W0l = W0_c.Value; Fl = F_c.Value;
    for ii = 1:numel(idxRange)
        ...
        for ll = 1:L
            Upsilon_local{ll} = diag(exp(1i*xi_bench{ll}));
        end
        Upsilon_local{l}(m,m) = exp(1i*xi_p);
...
title(sprintf('Training (no CST), $\\zeta$=%.3f, $f_c$=%.0f GHz (live)', zeta_fixed, fc/1e9), 'Interpreter','latex');
...
for it = start_it:maxIter
    ...
    parfor c = 1:nW
        ...
        for ii = 1:numel(idxRange)
            ...
            Upsilon_local = cell(L,1);
            for ll = 1:L
                Upsilon_local{ll} = diag(exp(1i*xi_cur{ll}));
            end
            Upsilon_local{l}(m,m) = exp(1i*xi_p);
    ...
    % ----- live feedback -----
    fprintf('iter %d/%d (%.1f%%), loss=%.4f, |beta|=%.4f, eta=%.4e, elapsed=%.1f s\n', ...
        it, maxIter, 100*it/maxIter, Lval, abs(beta), eta, toc(t_start));
    set(h_plot, 'XData', 1:it, 'YData', loss_hist(1:it));
    drawnow;

    % ----- periodic checkpoint -----
    if mod(it, checkpoint_every) == 0 || it == maxIter
        save(checkpoint_path, 'xi', 'Upsilon', 'loss_hist', 'beta_hist', ...
             'it', 'eta', 'zeta_fixed', 'maxIter', 'fc', '-v7.3');
        fprintf('  [checkpoint saved at iteration %d]\n', it);
    end

    % ----- periodic pool refresh (second safety net against cumulative -----
    % ----- pool-state issues over a long run, on top of the fresh start) -----
...
%% ----------------- Save final result -----------------
path = fullfile('..', 'Dataset', 'SIM_training_noCST_single_zeta_28GHz.mat');
save(path, 'xi', 'Upsilon', 'G', 'beta', 'loss_hist', 'beta_hist', ...
     'zeta_fixed', 'maxIter', 'eta0', 'seed', 'fc', '-v7.3');

% Release the workers now rather than waiting on IdleTimeout -- the
% client-side workspace (xi, loss_hist, etc.) is unaffected by this, only
% the worker processes are shut down.
delete(gcp('nocreate'));
