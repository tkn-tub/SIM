%% Training script — SIM Navigation with DQN, SIM2 Q-network (3 hidden M-layers)
% VARIANT with THREE hidden M-layers.
% Architecture: N -> [propN->M] -> M -> [propM->M] -> M -> [propM->M] -> M -> [propM->9] -> diode
% 691 trainable phases (16+225+225+225) vs 466 (2-hidden) and 241 (1-hidden).
% Saves to dqn_agent_navigation_SIM2_3hidden.mat -- the 1- and 2-hidden
% results are left untouched, giving a clean 1/2/3 M-layer ablation sweep.
%
% CAVEAT, now stronger: three phase masks separated only by fixed
% propagation, with the ONLY nonlinearity at the diode. The deeper this
% goes the more likely training is to stall or plateau no higher than
% shallower versions -- watch the gradient norms (verify script) and the
% learning curve. If 3-hidden does not beat 2-hidden, that is the
% reportable finding (diminishing/negative returns from all-phase depth),
% not a bug to fix.
%
...
save_path = fullfile('..', 'Dataset', 'dqn_agent_navigation_SIM2_3hidden.mat');
