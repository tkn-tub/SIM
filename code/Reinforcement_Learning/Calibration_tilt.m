...
function [psi_x, psi_y] = computePsiFromPos(pos_MU, EnvPars)
    % LOCAL-frame electrical angles: project the arrival direction onto the
    % tilted aperture's axes. R_tilt = I reproduces the legacy nadir mapping.
    d  = pos_MU(:) - EnvPars.pos_SIM(:);
    d  = d / norm(d);                     % global unit vector, SIM -> MU
    ul = EnvPars.R_tilt.' * d;            % [u1; u2; u3] in the aperture frame
