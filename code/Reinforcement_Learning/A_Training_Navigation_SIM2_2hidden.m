%% Training script — SIM Navigation with DQN, SIM2 Q-network (2 hidden M-layers)
% VARIANT of A_Training_Navigation_SIM2.m with a SECOND hidden M-layer.
% Architecture: N -> [propN->M] -> M -> [propM->M] -> M -> [propM->9] -> diode
% 466 trainable phases (16+225+225) vs 241 in the single-M baseline.
% Baseline file and its saved result (dqn_agent_navigation_SIM2.mat) are
% left untouched -- this saves to dqn_agent_navigation_SIM2_2hidden.mat so
% the two can be compared as a clean 1-vs-2 hidden-layer ablation.
%
% CAVEAT worth watching in the curve: each M-layer is a phase mask between
% fixed propagation matrices with NO nonlinearity between them (the only
% nonlinearity is the diode at the end). A deeper all-phase stack can be
% HARDER to train, not easier -- if this trains worse than the single-M
% baseline, that's a real property of the architecture, not a bug.
%
...
save_path = fullfile('..', 'Dataset', 'dqn_agent_navigation_SIM2_2hidden.mat');
